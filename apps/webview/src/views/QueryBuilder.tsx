import type { QueryFilter, QueryOperator, OrderByClause } from '@fireview/core';
...
export const QueryBuilder: React.FC<QueryBuilderProps> = ({ collections, onRunQuery, onClose }) => {
  ...
  const handleRunQuery = () => {
    ...
    const query: LocalFirestoreQuery = {
    ...
    >
      ...
      <div className="filter-group">
        ...
        <input
          ...
          onChange={e => setCollectionGroup(e.target.checked)}
        />
      </div>
        ...
        {filters.map((filter, index) => (
          <div key={index} className="filter-group" style={{ marginBottom: 8 }}>
            ...
            <select
              ...
              onChange={e => updateFilter(index, 'operator', e.target.value as QueryOperator)}
            ...
            />
            <button onClick={() => removeFilter(index)} className="btn-icon" title="Remove">
              ×
            </button>
          </div>
        ))}
        ...
        {orderBy.map((ob, index) => (
          <div key={index} className="filter-group" style={{ marginBottom: 8 }}>
            <input
              ...
              onChange={e =>
                setOrderBy(
                  orderBy.map((o, i) => (i === index ? { ...o, field: e.target.value } : o))
                )
              }
              className="field-input"
            />
            <select
              ...
              onChange={e =>
                setOrderBy(
                  orderBy.map((o, i) =>
                    i === index ? { ...o, direction: e.target.value as 'asc' | 'desc' } : o
                  )
                )
              }
            >
            ...
            <button onClick={() => removeOrderBy(index)} className="btn-icon" title="Remove">
              ×
            </button>
          </div>
        ))}
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
        <button onClick={handleRunQuery} style={{ flex: 1 }}>
          Run Query
        </button>
        <button

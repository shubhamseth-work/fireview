...
const RANGE_OPERATORS: QueryOperator[] = ['<', '<=', '>', '>='];
const EQUALITY_OPERATORS: QueryOperator[] = ['==', '!=', 'in', 'not-in'];
const ARRAY_OPERATORS: QueryOperator[] = ['array-contains', 'array-contains-any'];
...
export class QueryEngine {
  validateQuery(query: FirestoreQuery): ValidationResult {
    ...
    const fieldOperators = new Map<string, Set<QueryOperator>>();
  ...
  generateCode(query: FirestoreQuery, language: 'typescript' | 'javascript' | 'python'): string {
    const filtersCode = query.filters.map(f =>
      `.where("${f.field}", "${f.operator}", ${this.serializeValue(f.value, language)})`
    ).join('\n    ');

    const orderByCode = query.orderBy.map(ob =>
  ...
  private serializeValue(value: unknown, language: string): string {
    if (value === null) return language === 'python' ? 'None' : 'null';
    if (typeof value === 'string') return `"${value}"`;
    if (typeof value === 'number' || typeof value === 'boolean') return String(value);
    if (Array.isArray(value)) return `[${value.map(v => this.serializeValue(v, language)).join(', ')}]`;
